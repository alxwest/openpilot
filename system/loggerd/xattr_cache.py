...
def removexattr(path: str, attr_name: str) -> None:
  _cached_attributes.pop((path, attr_name), None)
  try:
    xattr.removexattr(path, attr_name)
  except OSError as e:
    if e.errno != errno.ENODATA and not (hasattr(errno, 'ENOATTR') and e.errno == errno.ENOATTR):
      raise
